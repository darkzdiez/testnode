	...
	jQuery.fn.tweetable = function (opts) {
		...
		return this.each(function () {
			...
			jQuery.getJSON(api + opts.username + limitcount + opts.limit).done(function(data) 
			{
				...
				if(twitterError)
				{
					tweetList.append('<li class="tweet_content item"><i class="fa fa-twitter"></i><p class="tweet_link">'+ opts.failed +'</p></li>');
					return;
				}
				...
				jQuery.each(data.tweets, function (i, tweet) {
					...
					tweetList.append('<li class="tweet_content_' + i + '"><i class="fa fa-twitter"></i><p class="tweet_link_' + i + '">' + tweet.response.replace(/#(.*?)(\s|$)/g, '<span class="hash">#$1 </span>').replace(/(\b(https?|ftp|file):\/\/[-A-Z0-9+&@#\/%?=~_|!:,.;]*[-A-Z0-9+&@#\/%=~_|])/ig, '<a href="$&">$&</a> ').replace(/@(.*?)(\s|\(|\)|$)/g, '<a href="http://twitter.com/$1">@$1 </a>$2').replace(/:">/, ' ">').replace(/: <\/a>/, '</a>:') + '</p></li>');
